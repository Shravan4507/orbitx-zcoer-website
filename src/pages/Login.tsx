import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useToast } from '../components/toast/Toast';
import { useAuth } from '../contexts/AuthContext';
import { signInWithGoogle } from '../services/firebase/auth';
import './Login.css';

export default function Login() {
    const navigate = useNavigate();
    const { showToast } = useToast();
    const { refreshProfile } = useAuth();
    const [isLoading, setIsLoading] = useState(false);

    const handleGoogleSignIn = async () => {
        setIsLoading(true);

        try {
            const result = await signInWithGoogle();

            if (result.isNewUser) {
                // New user - redirect to signup with prefilled email
                showToast('Please complete your profile to continue.', 'info');
                navigate('/signup', {
                    state: {
                        email: result.email,
                        isGoogleSignup: true
                    }
                });
            } else if (result.profile) {
                // Check if this is first login after signup
                const firstLoginName = localStorage.getItem('orbitx_first_login');

                if (firstLoginName) {
                    // First time login after signup
                    showToast(`Welcome, ${result.profile.firstName}!`, 'success');
                    localStorage.removeItem('orbitx_first_login');
                } else {
                    // Returning user
                    showToast(`Welcome back, ${result.profile.firstName}!`, 'success');
                }

                // Wait for auth state to propagate
                await refreshProfile();

                setTimeout(() => {
                    navigate('/user-dashboard', { replace: true });
                }, 100);
            }
        } catch (error: any) {
            console.error('Google sign-in error:', error);

            if (error.code === 'auth/popup-closed-by-user') {
                // User closed the popup, no need to show error
            } else if (error.code === 'auth/cancelled-popup-request') {
                // Multiple popup requests, ignore
            } else {
                showToast('Failed to sign in with Google. Please try again.', 'error');
            }
        } finally {
            setIsLoading(false);
        }
    };

    return (
        <main className="login-page page-transition">
            <div className="login-container login-container--compact">
                <div className="login-header">
                    <h1 className="login-title">Welcome to OrbitX</h1>
                    <p className="login-subtitle">Sign in or create an account to continue</p>
                </div>

                <div className="login-google-section">
                    <button
                        className="login-google-btn"
                        onClick={handleGoogleSignIn}
                        disabled={isLoading}
                    >
                        <svg viewBox="0 0 24 24">
                            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                        </svg>
                        {isLoading ? 'Please wait...' : 'Continue with Google'}
                    </button>

                    <p className="login-info">
                        By continuing, you agree to our Terms of Service and Privacy Policy
                    </p>
                </div>
            </div>
        </main>
    );
}
